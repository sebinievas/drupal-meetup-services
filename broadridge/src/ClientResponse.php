...
class ClientResponse extends Response {
   ...
   * {@inheritdoc}
   *
   * @throws ClientException if body cannot be json-decoded
   */
   ...
   * @return $this
   *
   * @throws Drupal\broadridge\ClientException
   */
  private function handleJsonResponse() {
    ...
    try {
      ...
      throw new ClientException($this, $e->getMessage(), $e->getCode(), $e);
    }

    if (empty($data)) {
      throw new ClientException($this, t('Invalid response'));
    }

    if (!empty($data['error'])) {
      throw new ClientException($this, $data['error']);
    }
    ...
    if (!empty($data['errorCode'])) {
      throw new ClientException($this, $data['errorCode']);
    }
    $this->data = $data;
    return $this;
  }
